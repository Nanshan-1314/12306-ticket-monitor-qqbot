...
>
>如果您擅自降低间隔、高频请求、绕过风控或以商业目的爬取，因此导致的 IP/账号封禁、法律纠纷或第三方索赔，责任全部由修改者/使用者独立承担，作者概不负责。
>
> 请善待 12306 公共资源。
...
```bash
momitor.exe #暂未实现 欢迎各位大佬push仓库到main分支 鉴于目前Agent发展迅速 暂不考虑喂饭 release或将在后续版本推出 也欢迎各位大佬fork项目
```
...
首次配置时选择 B 模式后，程序常驻监听 bot 私聊，
>注意：程序仅响应owner openid

可用指令：

| 命令 | 说明 |
|------|------|
| `/start` | 初始化车次信息（出发站 → 到达站 → 车次 → 日期 → 席别 → 间隔），完成后启动定时监控 |
| `/Refresh now` | 立即查询一次并回复；每 10 分钟最多 2 次；有票则发通知并停止监控 |
| `/Rate limit [int]` | 调整查询间隔（分钟，不得低于 10） |
| `/stop` | 停止定时监控（机器人保持在线） |
